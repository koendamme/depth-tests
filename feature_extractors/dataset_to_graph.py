import pickle
import os
import numpy as np
import matplotlib.pyplot as plt
from feature_extractors.us.extract_us_wave import get_wave_updated
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import json


for subject in ["A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"]:
    root = os.path.join("D:", os.sep, "Formatted_datasets", subject)

    with open(os.path.join(root, "mr.pickle"), 'rb') as file:
        mr = pickle.load(file)["images"]
    with open(os.path.join(root, "surrogates.pickle"), 'rb') as file:
        surrogates = pickle.load(file)
    with open(os.path.join(root, "mr_wave.pickle"), 'rb') as file:
        mr_wave = pickle.load(file)["mri_waveform"]
    with open(os.path.join(root, "mr2us.pickle"), 'rb') as file:
        mr2us = np.array(pickle.load(file)["mr2us"])
    with open(os.path.join(root, "settings.json"), 'r') as file:
        settings = json.load(file)

    us = surrogates["us"][mr2us[0]:mr2us[-1]]
    heat = surrogates["heat"][mr2us[0]:mr2us[-1]]
    coil = surrogates["coil"][mr2us[0]:mr2us[-1]]

    us.shape, heat.shape, coil.shape, len(mr)

    roi = settings["US"]["ROI"]
    us_wave = get_wave_updated(us, roi[0], roi[1], smooth=True)

    x_mr = np.linspace(0, 1, len(mr))
    x_surr = np.linspace(0, 1, us.shape[0])

    fig = make_subplots(rows=4, cols=1, shared_xaxes=True)

    fig.add_trace(go.Scatter(x=x_mr, y=mr_wave, mode="lines", name="MR"), row=1, col=1)
    fig.add_trace(go.Scatter(x=x_surr, y=us_wave, mode="lines", name="US"), row=2, col=1)
    fig.add_trace(go.Scatter(x=x_surr, y=coil, mode="lines", name="Coil"), row=3, col=1)
    fig.add_trace(go.Scatter(x=x_surr, y=heat, mode="lines", name="Heat"), row=4, col=1)
    fig.update_layout(height=800, width=600, title="Synchronized Surrogates")
    fig.write_html(f"D:/Formatted_datasets/Figures/{subject}.html")